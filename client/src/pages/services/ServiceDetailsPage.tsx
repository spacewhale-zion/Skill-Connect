...
const ServiceDetailsPage = () => {
  ...
  const fetchServiceAndTask = useCallback(async () => {
    ...
    try {
      ...
      if (
        associatedTask &&
        (associatedTask.taskSeeker._id === user?._id ||
          associatedTask.assignedProvider?._id === user?._id)
      ) {
      ...
      toast.error("Could not load service details.", { id: "error-toast" });
    } finally {
      setLoading(false);
    }
  }, [serviceId, user]);
  ...
  const handleSelectPaymentMethod = async (method: "Stripe" | "Cash") => {
    ...
    try {
      ...
      if (method === "Stripe" && res.clientSecret) {
        ...
        toast.success(
          "Service booked successfully! You can view it in your dashboard."
        );
        navigate("/dashboard");
      }
    } catch (error) {
  ...
  const handleProviderComplete = async () => {
    ...
    try {
      ...
      toast.error("Failed to mark service as complete.", { id: "error-toast" });
    }
  };
  ...
  const handleSeekerConfirm = async () => {
    ...
    try {
      ...
      setReviewee({
        id: task.assignedProvider._id,
        name: task.assignedProvider.name,
      });
      setIsReviewModalOpen(true);
    } catch (error: any) {
      toast.error(
                ...
                {isBooker && task?.status === "CompletedByProvider" && (
                  <div className="mt-6">
                    <p className="text-center text-sm text-slate-400 mb-2">
                      The provider has marked this service as complete.
                    </p>
                    <button
