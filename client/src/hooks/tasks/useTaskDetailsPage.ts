...
export const useTaskDetailsPage = () => {
  ...
  const [reviewee, setReviewee] = useState<{ id: string; name: string } | null>(
    null
  );
  ...
  const fetchTaskData = useCallback(async () => {
    ...
    try {
      ...
      if (
        user &&
        user._id === taskData.taskSeeker._id &&
        (taskData.status === "Open" || taskData.status === "Pending Payment")
      ) {
        const bidsData = await getBidsForTask(taskId);
        setBids(bidsData);
      }
    } catch (error) {
      toast.error("Failed to fetch task details.", { id: "error-toast" });
    } finally {
      setLoading(false);
    }
  }, [taskId, user]);
  ...
  const handleCancelTask = async () => {
    if (window.confirm("Are you sure you want to cancel this task?")) {
      try {
        ...
        toast.error("Failed to cancel task.", { id: "error-toast" });
      }
    }
  };
  ...
  const handleResumePayment = async () => {
    ...
    try {
      ...
      toast.error("Could not retrieve payment details.", { id: "error-toast" });
    }
  };
  ...
  const handleProviderComplete = async () => {
    ...
    try {
      ...
      if (updatedTask.taskSeeker) {
        setReviewee({
          id: updatedTask.taskSeeker._id,
          name: updatedTask.taskSeeker.name,
        });
        setIsReviewModalOpen(true);
      }
    } catch (error) {
      toast.error("Failed to mark task as complete.", { id: "error-toast" });
    }
  };
  ...
  const handleSeekerConfirm = async () => {
    ...
    try {
      ...
      if (updatedTask.assignedProvider) {
        setReviewee({
          id: updatedTask.assignedProvider._id,
          name: updatedTask.assignedProvider.name,
        });
        setIsReviewModalOpen(true);
      }
    } catch (error: any) {
  ...
  const isAssignedProvider =
    user && task && user._id === task.assignedProvider?._id;
  const chatRecipient = (
    isOwner ? task?.assignedProvider : task?.taskSeeker
  ) as AuthUser | undefined;
  const mapCoordinates: [number, number] | null = task
